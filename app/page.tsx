...
export default function Page() {
  ...
  const onlyPositiveNumber = (value: string) => {
    if (value === "") return "";
  ...
  const addItem = () => {
    ...
    if (!avgWorkMinutes || Number(avgWorkMinutes) <= 0) return setMessage("하루 평균작업시간은 양의 숫자로 입력해주세요.");
    if (!exposureMinutes || Number(exposureMinutes) <= 0) return setMessage("예상 하루 노출시간은 양의 숫자로 입력해주세요.");
  ...
  const submitData = async () => {
    ...
    if (!noApplicable && items.length === 0) {
      setMessage("작업 항목을 1개 이상 추가하거나 해당없음을 선택해주세요.");
      return;
    }
      ...
      <section style={styles.card}>
        ...
        <div style={styles.formGrid}>
          <label style={styles.label}>
            사업장 선택 <span style={styles.required}>*</span>
          ...
          <label style={styles.label}>
            부서 <span style={styles.required}>*</span>
            <input
              value={department}
          ...
          <div style={styles.formGrid}>
            ...
            <label style={styles.label}>
              ...
              <input
                ...
                onChange={(e) => setAvgWorkMinutes(onlyPositiveNumber(e.target.value))}
                inputMode="numeric"
                placeholder="양의 숫자만"
                style={styles.input}
              />
            ...
            <label style={styles.label}>
              ...
              <input
                ...
                onChange={(e) => setExposureMinutes(onlyPositiveNumber(e.target.value))}
                inputMode="numeric"
                placeholder="양의 숫자만"
                style={styles.input}
              />
